...
class _MyAppState extends State<MyApp> {
  ...
  final List<String> _modelVariants = ['tiny', 'tiny-en', 'base', 'small', 'medium', 'large-v2', 'large-v3'];
    ...
    _transcriptionSubscription = _flutterWhisperkitPlugin.transcriptionStream.listen(
      (result) {
        setState(() {
          ...
          final truncatedText = result.text.length > 30 ? '${result.text.substring(0, 30)}...' : result.text;
          _streamEvents.insert(0, 'Transcription: "$truncatedText" (Event #$_transcriptionEventCount)');
          if (_streamEvents.length > 10) {
            _streamEvents = _streamEvents.take(10).toList();
          }
        });
  ...
  Future<void> _transcribeFromFile() async {
    ...
    try {
      final filePath = await FilePicker.platform.pickFiles().then((file) => file?.files.firstOrNull?.path);
      ...
      final options = DecodingOptions(
        ...
        // Use selected language or null for auto-detection
        temperature: 0.0,
      ...
      final result = await _flutterWhisperkitPlugin.transcribeFromFile(filePath, options: options);
  ...
  Future<void> _toggleRecording() async {
    ...
    try {
      if (_isRecording) {
        ...
        _transcriptionSubscription = _flutterWhisperkitPlugin.transcriptionStream.listen((result) {
          setState(() {
            _transcriptionText = result.text;
            // Only add segments that don't already exist in the result
            for (final segment in result.segments) {
              if (!_transcriptionResult.any((existing) => existing.id == segment.id && existing.text == segment.text)) {
  ...
  Future<void> _detectLanguage() async {
    ...
    try {
      ...
      setState(() {
        ...
        _languageDetectionResult = LanguageDetectionResult(language: 'Error', probabilities: {'error': 1.0});
      });
    } finally {
      setState(() {
        _isDetectingLanguage = false;
  ...
  Future<void> _formatModelFiles() async {
    ...
    try {
      final result = await _flutterWhisperkitPlugin.formatModelFiles(_modelFilesToFormat);
      setState(() {
          ...
          children: [
            ...
            ModelConfigurationSection(
              ...
              onModelFilesChanged: (value) {
                setState(() {
                  _modelFilesToFormat = value.split(',').map((e) => e.trim()).where((e) => e.isNotEmpty).toList();
                });
              },
            ),
...
class AdditionalModelManagementSection extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        const Text('Additional Model Management', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
        Wrap(
          spacing: 8.0,
          children: [
            ElevatedButton(
...
class ResultApiTestingSection extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.stretch,
      children: [
        const Text('Result-based API Testing', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
        ElevatedButton(
...
class StreamMonitoringSection extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.stretch,
      children: [
        const Text('Stream Monitoring', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
        Text(
              ...
              streamEvents.isEmpty
                  ...
                  : ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      return Text(streamEvents[index], style: const TextStyle(fontSize: 12, fontFamily: 'monospace'));
                    },
                  ),
        ),
      ],
...
class LanguageSelectionDropdown extends StatelessWidget {
  const LanguageSelectionDropdown({super.key, required this.selectedLanguage, required this.onLanguageChanged})
    : _languages = const [
        'auto', // Auto-detect
        'en', // English
        'ja', // Japanese
          ...
          child: DropdownButton<String>(
            ...
            items:
                _languages.map<DropdownMenuItem<String>>((String value) {
                  ...
                  return DropdownMenuItem<String>(value: value, child: Text(displayText));
                }).toList(),
          ),
        ),
      ],
...
class ModelLoadingIndicator extends StatelessWidget {
  const ModelLoadingIndicator({super.key, required this.asyncLoadModel, required this.modelProgressStream});
        ...
        return StreamBuilder(
          ...
          builder: (context, snapshot) {
            ...
            if (snapshot.data?.fractionCompleted == 1.0) {
              return const Center(
                child: Column(spacing: 16.0, children: [CircularProgressIndicator(), Text('Model loaded')]),
              );
            }
            ...
            return Column(
              spacing: 16.0,
              children: [
                LinearProgressIndicator(value: snapshot.data?.fractionCompleted),
